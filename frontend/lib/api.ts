...
export const api = {
  ...
  generateSummary: async (leadId: string, content: string, token: string | null) => {
  ...
  generateReply: async (leadId: string, context: string, funMode: boolean, token: string | null) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || `Failed to generate reply: ${response.statusText}`);
    }
  ...
  refineReply: async (originalReply: string, instruction: string, token: string | null) => {
    ...
    if (!response.ok) {
      throw new Error(data.error || `Failed to refine reply: ${response.statusText}`);
    }
